// SDAPWA v1.1.0 - Mindfulness Screen (COMPLETE)
const MindfulnessScreen={
    ...
    stopBreathe(){
        ...
        const points=window.PointsManager?window.PointsManager.addBeing(1):null;
    },
    ...
    startMeditate(){
        ...
        const sound=document.getElementById('meditation-sound').value;
        if(sound){
            this.playMeditationSound(sound);
        }
        ...
        this.meditateTimer=setInterval(()=>{
            ...
            if(this.meditateTimeLeft<=0){
                this.stopMeditate();
                window.App.showToast('Meditation complete! +3 Being points','success');
            }
        },1000);
    },
    ...
    playMeditationSound(sound){
        ...
        if(soundUrls[sound]){
            this.audioElement=new Audio(soundUrls[sound]);
    ...
    async syncHealthData(){
        window.App.showToast('Syncing health data...','info');
        
        if('pedometer' in navigator){
            try{
                const result=await navigator.pedometer.queryPedometerData({startDate:new Date().setHours(0,0,0,0),endDate:Date.now()});
                const steps=result.steps||0;
                document.getElementById('steps-value').textContent=steps;
                await this.saveHealthData();
            }catch(e){
                console.log('Pedometer not available:',e);
                window.App.showToast('Automatic step tracking not available on this device','info');
            }
        }else{
            window.App.showToast('Automatic health sync not available on this device','info');
        }
    }
};
window.MindfulnessScreen=MindfulnessScreen;
console.log('✓ MindfulnessScreen loaded');
